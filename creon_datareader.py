...
class CreonDataReader:
    ...
    def __init__(self):
        ...
        # 88: ('당기순이익', ''), 89: ('BPS', ''), 
        # 90: ('영업이익증가율', ''), # 91: ('영업이익', ''), 
        # 92: ('매출액영업이익률', ''), # 93: ('매출액경상이익률', ''), 
        # 94: ('이자보상비율', ''), # 95: ('결산년월', ''), 
        # 96: ('분기BPS', ''), # 97: ('분기매출액증가율', ''), 
        # 98: ('분기영업이액증가율', ''), # 99: ('분기경상이익증가율', ''), 
        # 100: ('분기순이익증가율', ''), # 101: ('분기매출액(단위:원)', ''), 
        # 102: ('분기영업이익(단위:원)', ''), # 103: ('분기경상이익(단위:원)', ''), 
        # 104: ('분기당기순이익(단위:원)', ''), # 105: ('분개매출액영업이익률', ''), 
        # 106: ('분기매출액경상이익률', ''), # 107: ('분기ROE', ''), 
        # 108: ('분기이자보상비율', ''), # 109: ('분기유보율', ''), 
        # 110: ('분기부채비율', ''), # 111: ('최근분기년월', ''), 
        # 112: ('BASIS', ''), # 113: ('현지날짜', ''), 
        # 114: ('국가명', ''), # 115: ('ELW이론가', ''), 
        # 116: ('프로그램순매수', ''), # 117: ('당일외국인순매수잠정구분', ''), 
        # 118: ('당일외국인순매수', ''), # 119: ('당일기관순매수잠정구분', ''), 
        # 120: ('당일기관순매수', ''), # 121: ('전일외국인순매수', ''), 
        # 122: ('전일기관순매수', ''), # 123: ('SPS', ''), 
        # 124: ('CFPS', ''), # 125: ('EBITDA', ''), 
        # 126: ('신용잔고율', ''), # 127: ('공매도수량', ''), 
        # 128: ('공매도일자', ''), # 129: ('ELW e-기어링', ''), 
        # 130: ('ELW LP보유양', ''), # 131: ('ELW LP보유율', ''), 
        # 132: ('ELW LP Moneyness', ''), # 133: ('ELW LP Moneyness구분', ''), 
        # 134: ('ELW 감마', ''), # 135: ('ELW 기어링', ''), 
        # 136: ('ELW 내재변동성', ''), # 137: ('ELW 델타', ''), 
        # 138: ('ELW 발행수량', ''), # 139: ('ELW 베가', ''), 
        # 140: ('ELW 세타', ''), # 141: ('ELW 손익분기율', ''), 
        # 142: ('ELW 역사적변동성', ''), # 143: ('ELW 자본지지점', ''), 
        # 144: ('ELW 패리티', ''), # 145: ('ELW 프리미엄', ''), 
        # 146: ('ELW 베리어', ''), 
